...
def myTwFinder(userfun, cnfine, relate, refval, step, maxntw=200,nite=40,xtolerance=0.1,ytolerance=0.1):
    info=0

    def val(x):
        result = userfun(x)
        if isinstance(refval, bool):
            return 1 if result == refval else -1
        else:
            q = result - refval
            if relate == '<': q = -q
            return q

    if isinstance(refval, bool) and relate.upper() != 'EQUAL':
        raise Exception('When refval is boolean, relate must be "EQUAL".')
    elif not isinstance(refval, bool) and relate.upper() not in ('RANGE', '<', '>'):
