    ...
    def repopulate(self,gen): # Manel style # Tiene fallo
        ...
        for i in range(len(self.pop)):
            if i < ne:  # unchanged elite
                ...
                pop2[i] = copy.deepcopy(sorted_population[parent1][0])
                pop2[i].repFun(sorted_population[parent2][0], self.fit[i], self.fit[parent2]) #TO BE DONE: CALL WITH FITNESS & GENERATION !!!
                self.fit[i, :] = pop2[i].fitFun()
                popType2[i] = 'D'
